...
class Plotter:
    def __init__(self, root):
        ...
        for each_label in range(1, self.max_plot):
            self.labels_array[each_label] = Label(
                frame, text=str("y" + str(each_label) + "(x) = ")
            )
            self.labels_array[each_label].grid(
                row=self.first_row + 1 + each_label,
                column=self.first_column,
                sticky="e",
            )
        for each_entry in range(1, self.max_plot):
            self.entry_array[each_entry] = Entry(frame)
            self.entry_array[each_entry].grid(
                row=self.first_row + 1 + each_entry, column=self.first_column + 1
            )

        for each_color in range(1, self.max_plot):
            self.color_array[each_color] = Button(
                frame,
                text="█",
                fg="#0c00c0",
                command=lambda each_color=each_color: self.getColor(each_color),
            )
            self.color_array[each_color].grid(
                row=self.first_row + 1 + each_color, column=self.first_column + 2
            )

        for each_check in range(1, self.max_plot):
            self.check_array[each_check] = Checkbutton(
                frame, variable=self.checked[each_check]
            )
            self.check_array[each_check].grid(
                row=self.first_row + 1 + each_check, column=self.first_column + 3
            )
        ...
        self.line.grid(
            sticky="NS",
            row=self.first_row,
            column=self.first_column + 5,
            rowspan=1 + self.max_plot,
            pady=10,
        )
        ...
        self.label_graph_title.grid(
            row=self.first_row + 2, column=self.first_column + 6, sticky="e"
        )
        self.label_first_x.grid(
            row=self.first_row + 3, column=self.first_column + 6, sticky="e"
        )
        self.label_last_x.grid(
            row=self.first_row + 4, column=self.first_column + 6, sticky="e"
        )
        self.label_upper_y.grid(
            row=self.first_row + 5, column=self.first_column + 6, sticky="e"
        )
        self.label_lower_y.grid(
            row=self.first_row + 6, column=self.first_column + 6, sticky="e"
        )
        self.label_step.grid(
            row=self.first_row + 7, column=self.first_column + 6, sticky="e"
        )

        self.entry_graph_title.grid(
            row=self.first_row + 2, column=self.first_column + 7
        )
            ...
            for i in range(1, self.max_plot):
                if self.checkreadings[i] == 1:
                    pylab.plot(
                        self.xx[i],
                        self.yy[i],
                        label=str(self.entry_array[i].get()),
                        color=self.color_array[i]["fg"],
                    )
